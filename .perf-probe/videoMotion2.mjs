...
const mp4 = readFileSync(".perf-probe/out/rec2.mp4");
const server = createServer((req, res) => {
  if (!req.url.includes("rec2")) { res.writeHead(404); res.end(); return; }
...
await page.goto("http://localhost:4601/blank");
await page.setContent(`<video id="v" src="http://localhost:4601/rec2.mp4" muted playsinline></video>`);

const disp = await page.evaluate(async () => {
  const v = document.getElementById("v");
  await new Promise((res, rej) => { v.onloadeddata = res; v.onerror = () => rej(new Error("video load failed")); });
  console.log(`video ${v.videoWidth}x${v.videoHeight}, ${v.duration.toFixed(1)}s`);
  const W = 200;
  const H = Math.round((v.videoHeight / v.videoWidth) * W);
  const c = document.createElement("canvas"); c.width = W; c.height = H;
  const ctx = c.getContext("2d", { willReadFrequently: true });
  const y0 = Math.round(H * 0.28), y1 = Math.round(H * 0.44), rows = y1 - y0;
  const MAX = 26;
  const band = () => {
    ...
    for (let x = 0; x < W; x++) { let s = 0;
      for (let y = 0; y < rows; y++) { const i = (y * W + x) * 4; s += d[i]*0.299 + d[i+1]*0.587 + d[i+2]*0.114; }
      b[x] = s / rows; }
    return b;
  };
  const out = []; let prev = null;
  await new Promise((resolve) => {
    const done = setTimeout(() => { console.log("timeout guard"); resolve(); }, 90000);
    const step = (_n, meta) => {
      const cur = band();
      if (prev) {
      ...
      prev = cur;
      if (v.ended) { clearTimeout(done); resolve(); } else v.requestVideoFrameCallback(step);
    };
...
for (const r of rides) {
  ...
  for (let i = 2; i < it.length - 2; i++) {
    ...
    if (nb >= 3 && Math.abs(it[i][1]) <= nb * 0.4)
      stalls.push(`+${it[i][0]-r.start}ms: ${Math.abs(it[i][1])}px while neighbours ~${nb.toFixed(1)}px`);
  }
  console.log(stalls.length ? "  >>> STALL ON SCREEN:\n    " + stalls.join("\n    ") : "  clean");
}
