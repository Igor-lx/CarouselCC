...
const run=async(label,variant)=>{
  ...
  console.log(`${label.padEnd(56)} ${(tot/Math.max(1,n)).toFixed(2)}ms/frame`);
};
...
const mk=(minScale)=>Array.from({length:33},(_,k)=>{const t=k/32; const sc=1-(1-minScale)*t;
  return {transform:'translate3d('+(k*2)+'px,0,0) scale('+sc+')', opacity:1-t};});`;

await run("A) widget's own (reaches scale(0))", null);
await run("B) synthetic, scale 1 -> 0.36 (never 0)", new Function(`return ()=>{ ${KILL}
  dots.forEach(el=>el.animate(mk(0.36),{duration:1800,fill:'both'})); }`)());
await run("C) synthetic, scale 1 -> 0  (EXACT zero)", new Function(`return ()=>{ ${KILL}
  dots.forEach(el=>el.animate(mk(0),{duration:1800,fill:'both'})); }`)());
await run("D) synthetic, scale 1 -> 0.0001 (clamped)", new Function(`return ()=>{ ${KILL}
  dots.forEach(el=>el.animate(mk(0.0001),{duration:1800,fill:'both'})); }`)());
await browser.close();
