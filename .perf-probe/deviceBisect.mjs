...
const run=async(label,variant)=>{
  ...
  console.log(`${label.padEnd(48)} ${(tot/Math.max(1,n)).toFixed(2)}ms per frame`);
};
...
// Append 11 identical animated divs into a chosen host, to see which ancestor kills compositing.
const probeIn = (hostExpr) => new Function(`return ()=>{
  ${KILL}
  const host = ${hostExpr};
  for(let i=0;i<11;i++){
    const d=document.createElement('div');
    d.style.cssText='position:absolute;top:0;left:0;width:24px;height:24px;border-radius:50%;background:transparent;will-change:transform,opacity';
    host.appendChild(d);
    d.animate([{transform:'translate3d(0px,0,0)'},{transform:'translate3d(60px,0,0)'}],{duration:1800,fill:'both'});
  }
}`)();

await run("0) no animations (floor)", new Function(`return ()=>{ ${KILL} }`)());
await run("1) test divs in <body>", probeIn("document.body"));
await run("2) test divs in the carousel ROOT", probeIn("document.querySelector('[data-carousel-root]')"));
await run("3) test divs in the WIDGET container", probeIn("document.querySelector('[class*=\"container_PW\"]')"));
await browser.close();
