...
await run("A) widget's own (33kf + explicit startTime)", null);
await run("B) synthetic 33kf, NO startTime", new Function(`return ()=>{ ${KILL}
  dots.forEach(el=>el.animate(KF,{duration:1800,fill:'both'})); }`)());
await run("C) synthetic 33kf, WITH explicit past startTime", new Function(`return ()=>{ ${KILL}
  dots.forEach(el=>{ const a=el.animate(KF,{duration:1800,fill:'both'}); a.startTime=document.timeline.currentTime-40; }); }`)());
await run("D) synthetic 33kf, fill:'none'", new Function(`return ()=>{ ${KILL}
  dots.forEach(el=>el.animate(KF,{duration:1800,fill:'none'})); }`)());
await browser.close();
