...
if (mode === "tested") {
  ...
  if (changed !== null) {
    ...
    // графе импортов.
    // Расширение фильтруется намеренно: область задана путями, а по пути в неё
    // попадает и README движка, который браузер сломать не может. Требование
    // прогона на правке документации — это прогон, который про неё ничего не
    // докажет, а канал, кричащий не по делу, перестают читать; ровно этого
    // конфиг области и обещал избежать. Стили остаются: они в графе импортов
    // не видны, но краску меняют. Найдено пробой.
...
if (mode === "verify") {
  ...
  if (CONFIG.toolchain != null && CONFIG.manifest != null) {
    ...
    if (gaps.length) {
      ...
      console.log(
        "  на полке. Установленный инструмент без настройки хуже отсутствующего:",
      );
      console.log("  цепочка зеленеет, не проверяя ничего.");
    }
  }
