...
export interface PointerSwipeConfig {
   ...
   * every sample — the engine has no notion of edges. `0` = 1:1 tracking, and
   * a number outside the range is clamped to the nearest end. A value that is
   * not a number is refused at the door instead: see `usePointerSwipe`. */
  resistance?: number;
