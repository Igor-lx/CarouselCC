...
| [`context/`](./context/docs/README.md) | **Not code.** The working rules and the graph tool, planted in a new project before its first line: the quality bar, the workflow loop, and how to keep a knowledge base alongside the code. Start with `docs/README.md` — written for a person, in the language this repo uses for documents outside `src`. |
