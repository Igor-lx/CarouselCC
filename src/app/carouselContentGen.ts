import { buildResponsiveSlide } from "../components/Carousel/contract/buildResponsiveSlide";
import type { Slide } from "../components/Carousel/contract/types";

/**
 * Pure core of the carousel-data generator (no fs, no DOM) — exported so it can
 * be unit-tested and driven by `scripts/gen-carousel-data.ts`.
 *
 * Turns per-resolution asset maps into the `Slide[]` content document the
 * carousel consumes, and — crucially — does an **idempotent merge** against the
 * previous document: a slide is identified by its stable slug (the asset
 * filename), so regeneration preserves the human-authored `alt` and the
 * existing `id`, mints ids only for genuinely new assets, and drops removed
 * ones. That keeps the artifact safe to regenerate without losing editorial
 * work or churning identities.
 */
...
export interface GenerateCarouselSlidesInput {
...
const candidatesAt = (widths: GenVariantWidth[], slug: string) =>
