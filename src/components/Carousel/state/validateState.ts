...
export const validateCarouselState = (
  ...
  if (state.teleportVirtualIndex !== null && state.motionPhase !== "step-jump") {
    issues.push({
      ...
      consequence:
        "MOTION_SETTLED may interpret this as a pending far-GO_TO teleport in the wrong context",
    });
  }
  ...
  if (state.isTeleportApproach && state.motionPhase !== "step-jump") {
    issues.push({
      ...
      consequence:
        "segmentFactory builds a teleport-approach profile in the wrong phase",
    });
  }
