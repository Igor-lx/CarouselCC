...
export const collectViewportAxisWarnings = (): CarouselDiagnosticWarning[] => {
...
export const collectSlideSourceMediaWarnings = (
  ...
  return [...offending].map((media) => ({
    ...
    actual: media,
    expected:
      "One of the canonical axis strings (SLIDE_CANONICAL_SOURCE_MEDIA)",
    consequence:
...
export const collectViewportCssWarnings = (): CarouselDiagnosticWarning[] => {
  ...
  for (const sheet of document.styleSheets) {
    ...
    walkRules(rules, (selector) => {
      ...
      for (const name of collectSelectorStateNames(
        selector,
        "data-breakpoint",
      )) {
  ...
  for (const name of breakpointNames) {
    ...
    if (!referencedBreakpoints.has(name)) {
      out.push({
        ...
        actual: name,
        expected:
          "Referenced from at least one stylesheet (or deliberately unstyled)",
        consequence:
