...
| Autoplay lifecycle | `useCarouselAutoplay` over `useAutoplay` | Adapter owning viewport visibility, the pause rule, and stable step handlers over the interval loop. Same *shape* as the gesture pair, different *tier*: the pointer-swipe primitive is a reusable engine in `shared`, while both autoplay halves are deliberately component-local — an internal redistribution for clarity, not a shared engine. |
...
**ADR-003 — one compositor path for every planned motion.** The first
reaction of a fresh reader is legitimate: *"it's a one-page step on a
button — `transition: transform` plus one `translateX(...)` write would do;
why the keyframe machinery?"* This ADR exists to answer exactly that. The
naive version fails four separate ways, and each failure is something this
carousel visibly does:

1. **The curve.** A CSS transition eases by a `transition-timing-function`,
   and the engine's accel/cruise/decel profile (distance shares) is not
   expressible as one — no bezier holds a flat cruise between two ramps.
   The "simple" step would therefore move on a SECOND, approximated curve:
   two sources of truth for how one step feels, drifting apart with every
   tuning change, and a step that feels different from the autoplay step,
   the release glide and the jump it sits next to.
2. **The retarget.** Click again mid-flight and a transition restarts
   toward the new value with the same easing from a standstill — a visible
   velocity kink, because a transition carries no notion of current speed.
   The engine rebuilds the segment from a `(position, velocity)` handoff
   and the compositor keyframes reproduce it, so a second click continues
   the motion instead of rebooting it. Repeated-click pickup (§6) is built
   on exactly this.
3. **The chorus.** The track is not the only thing moving: the pagination
   widget steps and the dot cross-fade run over the SAME curve and the SAME
   clock (`startedAt` pinning, §4.5). A CSS transition has no shareable
   plan and no settable start time — the followers would have to guess at
   its phase. And `transitionend` as a settle signal is famously lossy
   (interrupted/canceled transitions never fire it), while the controller's
   settle is deterministic.
4. **The jank.** The original start-of-motion freeze was main-thread
   contention, and it hit the single click step hardest — a step begins
   exactly when the main thread is busiest (React commits the expanded
   render window, images decode). `transition` on `transform` is
   compositor-driven too, so this point alone would not reject it — but
   points 1–3 already have, and the pre-WAAPI per-frame JS writes it would
   replace are how the jank was born in the first place.

So the keyframe transport is not ceremony around a `translateX` — it is the
minimum machinery that keeps one curve, continuous retargets, a
