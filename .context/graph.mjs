...
if (mode === "tested") {
  ...
  if (changed !== null) {
    ...
    // Сверка с планкой — единственный обязательный шаг закрытия, у которого не
    // было машинного напоминания: у тестов есть этот же режим, у базы `verify`,
    ...
    if (touchedCode.length || touchedStyles.length) {
      ...
      // Второй вопрос того же момента, и он про другое: планка — про свой дифф,
      // этот — про то, что попалось рядом. Единственная форма откладывания,
