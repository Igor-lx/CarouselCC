...
| База знаний | `node .context/graph.mjs verify` | **exit 0** (все шесть проверок) |
...
| --- | --- | --- |
| `dist/assets/index-*.js` | 282.88 кБ | **88.82 кБ** |
...
`verify` делает четыре проверки и падает, если провалилась любая:
...
| объявленные размеры | `(N)` у файла и `(N файлов, M)` у папки совпадают с диском |
...
исключены: в них нет ни поведения, ни связей. Размер папки считается по коду без
