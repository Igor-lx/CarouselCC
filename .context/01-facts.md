...
| `@playwright/test` (dev-зависимость) | управление настоящим Chromium: снимки, чтение DOM, эмуляция устройств, троттлинг CPU, трассировка композитора через DevTools Protocol. Ставится **не как часть набора проверок** — своего npm-скрипта у неё нет и `npm test` её не гоняет; это инструмент под разовый замер и осмотр глазами. Браузеры лежат в общем кэше `%LOCALAPPDATA%ms-playwright` (703 МБ: chromium 428, headless shell 272, ffmpeg 3.4) — он **вне репозитория и общий для всех проектов**, поэтому в новом проекте нужен только `npm i -D` |
