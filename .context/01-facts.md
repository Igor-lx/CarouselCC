...
4. **PENDING** — `react-hooks/refs` выключено для **одного файла**,
   `useCarouselState.ts` (список в `eslint.config.js`). Было шесть.
...
| `useCarouselState` | **остаётся**, решение заказчика — вариант (b), см. `02-todo.md` |
