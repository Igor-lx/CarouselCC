...
| Файл | Что задаёт |
| --- | --- |
| `stryker.config.json` | мутационный прогон: область — `client/{domain,state,config/resolve}/**` без бочек, типов и тестов; таблицы констант (`config/*.ts`) вне области намеренно — их мутанты убиваются только пересказом самих значений; раннер vitest, анализ покрытия `perTest`, порог не рвёт сборку (`break: null`) — это аудит, а не ворота |
