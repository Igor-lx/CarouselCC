...
`verify` намеренно нестрогий в одну сторону: файл засчитывается упомянутым по
имени с расширением **или** без него — карта пишет и полными именами, и группами
вида `{a,b}.ts`. Ambient-объявления (`*.d.ts`) из проверки исключены: в них нет
ни поведения, ни связей. Якоря проверяются по всем `*.md` базы: файл должен
существовать, а номер строки — не выходить за его длину.
