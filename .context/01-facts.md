...
| найденное исправлено | в коде нет маркеров отложенной работы (`TODO`, `FIXME`, `HACK`, `XXX`, `ВРЕМЕННО`, `ПОТОМ`, `ПОЧИНИТЬ`), и ни один абзац базы не откладывает дефект без ссылки на `09-decisions.md`. Ловятся обе **записанные** формы откладывания; молчание — находку, которую просто не записали, — машина не видит, и это остаётся на ревью |
