...
| Мутации (по требованию) | `npm run test:mutation` | последний полный замер — **92.88 %** (убито 1848, выжило 141, без покрытия 1, таймаутов 3 из 1993), но он сделан **до** включения жеста в область: `stryker.config.json` с тех пор расширен на `client/gesture/**` и `shared/engines/gesture/**`, и следующий полный прогон даст другое число. Отдельный замер по жесту — **81.23 %** (убито 610, выжило 139, без покрытия 2, ошибок 2), ~15 минут. Все выжившие разобраны поимённо: домен и состояние — `08-tests.md`, § E (31), ядро движения — там же, § G (111), распознавание жеста — § H2 (143), DOM и геометрия — § H3 (144), обвязка и презентация — § H4 (552). Отдельный замер по DOM и геометрии — **83.12 %** (убито 708, выжило 140, без покрытия 4, таймаут 1), ~11 минут; по обвязке и презентации — **71.98 %** (убито 1406, таймаутов 12, выжило 509, без покрытия 43 из 1970 засчитанных; 165 строковых мутантов помечены `Ignored` и в знаменатель не входят), ~20 минут |
