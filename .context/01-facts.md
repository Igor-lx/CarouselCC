...
| Тесты | `npm test` | **112 файлов / 992 теста — все зелёные** |
| Сборка | `npm run build` | **exit 0**, 227 модулей |
| Линт | `npm run lint` | **exit 0** |
| Формат | `npm run format:check` | **exit 0** |
| База знаний | `node .context/graph.mjs verify` | **exit 0** (все восемь проверок) |
| Покрытие | `npm run test:coverage` | строки **88.70 %** (3753/4231), ветки **78.99 %** (2004/2537), функции **87.41 %** (806/922) |
...
| --- | --- | --- |
| `dist/assets/index-*.js` | 283.14 кБ | **88.90 кБ** |
...
`verify` делает восемь проверок и падает, если провалилась любая:
