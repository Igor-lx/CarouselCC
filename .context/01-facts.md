...
| Мутации (по требованию) | `npm run test:mutation` | **92.88 %**: убито 1848, выжило 141, без покрытия 1, таймаутов 3 из 1993. Прогон ~25 минут. Область — `domain`, `state`, `config/resolve`, `client/motion`, `client/visual-position`, `engines/motion`. Все выжившие разобраны поимённо: домен и состояние — `08-tests.md`, § E (31), ядро движения — там же, § G (111) |
