...
## Проект на одной странице

Сжатая модель для тех задач, где полная карта не нужна: её читают вместо
полутора тысяч строк ниже, а к разделам спускаются уже по адресу. Всё
перечисленное здесь разобрано подробно там же, ниже, и в `docs/architecture/`.

**Порядок слоёв** (снизу вверх, импорты идут только в эту сторону):

```
domain ──> state ──> motion / visual-position ──> geometry ──> компоненты
   └──────────────────────────────────────────> modules, slides, presentation
config ──> всё, что читает настройки
shared/** ─ полки: импортируются откуда угодно, о карусели не знают
```

Единственная зависимость вверх — из слоя `state` в файл `motion/timing.ts`
(тянут его `state/reducer.ts` и `state/transitions.ts`), и она описана отдельно.
Формулировка «два файла `motion/timing`» стояла здесь и читалась наоборот: два —
это потребители, а цель одна, и `timing` — файл, а не папка. Цикл в проекте ровно один, известный, и держится на том, что обратное
ребро — **только тип**.

**Пять источников истины**, каждый с одним владельцем и без зеркал:

| Что | Владелец |
| --- | --- |
| логическое состояние (цель, фаза, жест, причина движения) | `state/useCarouselState`, редьюсер, времени не знает |
| визуальная позиция «где трек прямо сейчас» | `visual-position/useVisualPosition` — сюда подписывается всё покадровое |
| факты раскладки | `slides/useCarouselSlideDeck` |
| рантайм-конфиг | `config/resolve/useCarouselConfig` — подставляет умолчания только на `undefined` |
| ресурсы картинок | стор на карусель, наблюдение, в навигацию и движение не вмешивается |

Состояние никогда не читает сэмпл движения; визуальная позиция никогда не
читает состояние; мост между ними ровно один — раннер движения.

**Четыре потребителя кадров**: трек (`geometry/useTrackBinding`), общая машина
следования пагинации (`modules/Pagination/useOffsetFollow`, за ней обе
реализации точек) и раздача в контексты из `Carousel.tsx`. Правило сброса
кадров в JS-фолбэке общее — иначе они разъедутся между собой.

**Парные копии** (правятся вместе, сверяются режимом `twins`):
`shared/engines/{motion,gesture}` против `shared/engines/kinetic/internal/*`;
`shared/clientState/**` — `library/*` против `useMedia`/`useUserEnvironment`.

**Что снаружи**: хост передаёт слайды, класс, окружение (`reducedMotion`,
`touch`, `dataSaver`) и получает снимок состояния колбэком. Окружение карусель
**никогда не определяет сама**. Входы caller-owned и не чинятся (ADR-002).
...
`motion.ts` — доли профилей, скоростные
...
`client/state/**` — 11 импортёров (+10 тестовых). Важны два вида зависимости —
...
Своё плюс **реэкспорт из полки** `shared/engines/motion` — `keyframesAlongStops`,
`positionAtNow`, `startPinnedAnimation` и тип `InFlightSpan`. Критерий отбора
задан комментарием `:10-12` `Only what the modules actually take` — у модулей
карусели один корень импорта для всего, что касается движения, но бочка при
этом дверь, а не витрина: полка отдаёт заметно больше, и остальное сюда не
поднимается.
...
- Оповещение идёт по снимку набора с проверкой членства: подписавшийся во время
  ...
  эмиттерах проекта без исключений — идиома описана в `10-idioms.md`, раздел
  «Рассылка идёт по снимку набора, с перепроверкой членства», и проверяется
  перебором, а не счётом: счёт здесь стоял и разошёлся. Для полки это часть
...
`client/visual-position/**` — 7 импортёров (+6 тестовых), из них рабочих
четыре: трек (`geometry/useTrackBinding`), общая машина следования пагинации
(`modules/Pagination/useOffsetFollow`, за ней обе реализации точек) и
`Carousel.tsx` (раздача в контексты). Правило сброса кадров читают двое —
трек напрямую, обе пагинации через одну машину.
...
- До первого замера отдаётся пол дистанции коммита: настоящий жест всегда
  переживает первое измерение. «Не измерено» здесь — **всё, что не конечное
  положительное число** (`isPositiveFinite`), а не только ноль и отсутствие.
  Прежняя форма охраны (`!(slotPx > 0)`) резала ноль, отрицательное и `NaN`, но
  пропускала `Infinity` как настоящий слот, и колода уезжала на бесконечной
  дистанции; ветку держит тест слоя.
...
- **`layoutOrigin`** — пере-базируется лишь при дрейфе за
  ...
  Начальное значение **засеяно окном того же монтирования**, а не `null`:
  состояние «ещё не решено» жило бы ровно один рендер и стоило бы **целого
  лишнего прохода по колоде** до первой отрисовки (замерено: 54 итерации `map`
  против 27). Отсюда и тип `number`, а не `number | null`;
- **кэш объектов `VirtualSlide`** — по виртуальному индексу, с двумя
  `eslint-disable`. Прежний `CONSTRAINT` обещал, что без него перерисовывается
...
#### `Pagination/useOffsetFollow.ts` — покадровое следование, общее на две реализации
Дельта-следование в **собственной шкале** следящего: виджет считает шагами,
точки — страницами, колода — виртуальными индексами, которые заворачиваются.
Поэтому follow привязывается к первому увиденному кадру и дальше идёт **разницей**
от этой привязки: захват на ходу оставляет ленту там, куда она дошла, вместо
прыжка на число колоды.
- Владельцев решения по одному: якорь сбрасывает **только** `startFollowing`,
  `stopFollowing` его не трогает.
- Правило сброса кадров читается из рефа **на каждом кадре**, а не берётся из
  замыкания: отпущенный палец меняет вид поездки без новой подписки.
- Позиция двигается и на сброшенном кадре — сбрасывается **краска**, не место.
- `geometry/useTrackBinding` третьим потребителем **не является**: он живёт в
  шкале колоды, пишет абсолютную позицию и подписан один раз на всю жизнь.
...
- **Внешний вид принадлежит CSS**: `readDotStates` читает те же переменные
  ...
  **обязаны зеркалить** `Pagination.module.scss`, и держит это не внимание, а
  `basic/tests/fadeFallbackSync.test.ts`: он читает оба файла и сверяет числа.
  Сами значения здесь не приводятся — это была бы третья их копия, и, в отличие
  от первых двух, её не держало бы ничто.
...
- Пункт назначения развёртки **не пропускается никогда** — на его финише висит
  посадка. Сегодня страж недостижим (лента кончается на этой точке, и её сила
  доходит до единицы на последнем стопе), но его отказ оставил бы точки
  захваченными навсегда, поэтому он остаётся страховкой, а сам инвариант
  закреплён тестом по каждой странице.
- Само следование — не своё: хук отдаёт `useOffsetFollow` три вещи (чем читать
  живую позицию, что отпустить при захвате, чем красить) и не держит ни подписки,
  ни якоря.
...
**Два режима отрисовки, и почему подсветка — отдельный слой.** У точки два
независимых числа: собственное затухание (`opacity` по расстоянию от центра) и
подсветка (`activeStrength`, единица только в центре, ноль за один полный шаг).
Одному элементу двух прозрачностей не дать, поэтому режимы решают это по-разному,
а стили переключаются по `data-motion-bound`:

- **статический** — подсветка это `::after` самой точки, её прозрачность берётся
  из переменной `--dot-active-strength`, которую React пишет инлайном;
- **связанный** — этот `::after` **выключается**
  (`.container_PW[data-motion-bound="true"] .dot_PW::after { display: none }`), а
  подсветку дают пуловые оверлеи `activeDot_PW`, которые красит биндинг и отдаёт
  компоузитору. Настоящий элемент — единственное, чему вообще можно задать
  keyframes по прозрачности.

Оверлеи **не дубликат точек**. В покое живой ровно один: лента всегда садится на
целое смещение, значит точка в центре есть, и оверлей стоит на ней с силой 1. На
полушаге точки в центре **нет вовсе** — при умолчаниях соседи стоят на `±24.5 px`,
и подсветка перетекает между ними по половине силы. Прибить одно свечение к
центру значило бы светить в зазор между двумя точками.
...
- Следование — **дельтой** в собственной шкале шагов виджета, и живёт оно в
  общем `useOffsetFollow`, а не здесь: своё у виджета только то, что он отпускает
  при захвате (память о прерванном шаге, отмена анимаций, сброс кэшей письма).
...
### L5. `modules/Diagnostic` — dev-only слой
...
- `checks/constantChecks.ts` — проверки руками написанных констант (счёт здесь
  не приводится: он растёт с каждой новой константой и ловить его нечем):
...
- Читает `?slides` **на модульном уровне** (`App.tsx:38-43` `const SLIDES_SET`) — SSR-враждебно, для
...
проекта — `shared/**` — 32 импортёра (+4 тестовых).
**Проверено на коллизии:** `kinetic/index.ts` отдаёт узкую поверхность — хук,
таблицу умолчаний и типы к ним, — внутренние форки наружу не выходят; `motion` и
`gesture` не пересекаются по именам. Счёт имён здесь не пишется: он едет от
каждой правки фасада и уже разошёлся однажды. Неоднозначных
...
- `swipe/usePointerSwipe.ts` — распознавание: фазы
  `idle → press → dragging → cooldown`, окно перехвата — `catchDelayMs`
...
| | Что расходится |
| --- | --- |
| `motion` | `compositedRide.ts` и `index.ts` |
| `gesture` | `index.ts`; нет `releaseKinetics.ts` и его теста |

Счёта файлов здесь нет намеренно: он ехал от каждой правки и уже соврал — стояло
«20 против 20» и «17 против 15» при других числах на диске. Нужен счёт — его
даёт `graph.mjs sizes`, а список пар — `CONFIG.forks`.

Всё остальное — включая `createMotionController`, `profile.ts`,
`progressCurve.ts`, `usePointerSwipe` — **байт-идентично** после нормализации
комментариев с путями к докам. Объём этих файлов тоже не заявляется: стояло «297
`297 строк` и `651 строка` при `306 и 660` на диске, и ловить такое нечем.
...
- `useIsomorphicLayoutEffect.ts` — «спящая» локальная копия для выноса полки;
  ...
  числится среди файлов без импортёров (`01-facts.md`, C). **Это пара, и она
  объявлена машине** (`CONFIG.forks`): правка канона обязана дойти до копии, и
  `twins` теперь об этом спрашивает. До пробы не спрашивал — правило про парные
  копии было, а машины за ним для этой пары не стояло.
...
- `environment.md` + `settings.template.json` — **среда**: что вызывает запрос
  подтверждения, что снимается списком разрешений, а что нет, раздел труда
  между оболочкой и инструментами поиска и политика концов строк. Правила и
  инструмент переносятся копированием, а среда — нет, и её отсутствие платится
  не красным прогоном, а простоем: длинная задача встаёт на подтверждении и
  ждёт человека. Что проект не потерял обещанного шаблоном, сверяет `verify`
  («Разрешения среды»).
- `stryker.template.json` — скелет мутационного прогона: `coverageAnalysis`,
  отключённый порог и исключение строковых мутантов. Машинной сверки с
  проектным конфигом нет намеренно: общего у них только эти три настройки, а
  область (`mutate`) у каждого проекта своя, и сверка на совпадение врала бы.
...
- `math/numeric.ts` — гарды вида `(value: unknown) => value is
...
Полностью изолирован: `node:*` + собственные типы,
...
`<Carousel>` и проверяет проводку хуков между собой. Комментарий
...
- Тесты в `engines/kinetic/internal/**/tests/` — **байт-в-байт копии**
  ...
  Единственное исключение — `compositedRide.test.ts`, он короче оригинала:
  форк осознанно урезан вслед за урезанной реализацией.
- `environment/library/tests/*` против `useUserEnvironment/tests/*` — отличие
  в несколько строк: заголовок + путь импорта. Комментарий фиксирует причину:
...
**байт-идентичны** (`../README.md` → `shared/engines/gesture/README.md`). Логика не
разъехалась. Единственное реальное расхождение —
`compositedRide.ts`: форк заметно короче оригинала, потому что форк
...
5. **`engines/kinetic` не имеет ни одного потребителя.** Grep по всему `src`:
   ...
   он реэкспортируется, но никем не берётся. Весь фасад с его тестами —
