...
### `client/presentation/useCarouselPresentation.ts` (83) — fx (кэш в рефе)
...
### `client/slides/useSlideRenderModel.ts` (151) — fx (2 рефа-кэша)
...
### `client/slides/useSlideFetchReach.ts` (98) — fx
...
- `compositor/compositedRide.ts` (262) — «под ключ» путь для ОДНОГО элемента.
