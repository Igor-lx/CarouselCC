...
- `environment.md` + `settings.template.json` — **среда**: что вызывает запрос
  подтверждения, что снимается списком разрешений, а что нет, и раздел труда
  между оболочкой и инструментами поиска. Правила и инструмент переносятся
  копированием, а среда — нет, и её отсутствие платится не красным прогоном, а
  простоем: длинная задача встаёт на подтверждении и ждёт человека. Что проект
  не потерял обещанного шаблоном, сверяет `verify` («Разрешения среды»).
