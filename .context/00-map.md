...
Структурное сравнение детей, потому что JSX-слоты — свежие объекты на каждый
рендер хоста. Без него любой ре-рендер хоста реконсилирует всю колоду
(«Do NOT remove»).
- Дети сравниваются по типу, ключу и **поверхностно по пропам**, с рекурсией до
  `MAX_CHILD_COMPARE_DEPTH = 4`.
- Предохранитель **fail-safe** (`:9-15`): глубже лимита возвращается «изменилось».
  Худший исход — один лишний ре-рендер, никогда не пропущенный.
- Остальные пропы — по идентичности.

### `client/index.ts` (13), `public-api/**` (138)
...
### `data-gen/**` (359 + 142 README) — генератор контента
Node-only кит, **полностью самодостаточный**: только встроенные модули Node,
ничего из компонента, поэтому папку можно скопировать на сервер и запустить.
- Собственный тип `GeneratedSlide` объявлен локально и намеренно: он узкий,
  JSON-сериализуемый подмножество `Slide` компонента.
- `runDataGen.ts` (131) — единственная часть, которая трогает диск; всё
  остальное чистое.
- `generateSlides.ts` (90) — идемпотентность: повторный запуск на тех же файлах
  даёт тот же документ.
- `index.ts` — **мёртвая бочка**: `cli.ts` берёт `runDataGen` напрямую.
...
- `media/library/*` (5 файлов) — форк тех же примитивов, но **на том же
  `useMediaQuery`**: стор общий, дублируются только обёртки.
...
`try/catch`, синхронизация между вкладками через событие `storage`,
`data-theme` на `<html>` в layout-эффекте. Цвета и ключ дублированы в
