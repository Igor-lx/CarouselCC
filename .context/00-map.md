...
### `client/slots/slotNames.ts` + `index.ts` (16)
...
- `Carousel.module.scss` (223) в `@layer baseStyles`; модули — в
...
- Читает `?slides` **на модульном уровне** (`:35-40`) — SSR-враждебно, для
...
- `App.module.scss` (118) — сырые `@media` вместо осей: это хост, у него свои
  оси, формально допустимо.
- `globals.scss` (135) — токены темы, `@layer reset`, объявление порядка слоёв.
...
### `shared/engines/motion/` (13 файлов, 1159) — движок движения
...
- `runtime/types.ts` (88) — контракт целиком. Две вещи объявлены **раздельно и
  ...
  **испущенный** кадр — для UI, не для передачи движения»). Смешать их —
  ...
  Комментарий `:41-43` объясняет, почему опциональные поля объявлены как
...
- `profile/profile.ts` (183) — зоны разгона, круиза и торможения; скорость внутри
  ...
  **Доли берутся как есть** (`:92-93`): перерасход даёт отрицательный круиз, и
  зона просто пропускается — это ADR-002 в действии, никакой нормализации.
...
- `compositor/compositedRide.ts` (268) — «под ключ» путь для ОДНОГО элемента.
  ...
  анимации. Комментарий `:19-22` отделяет этот случай от «раздать многим» —
  там композируют примитивы напрямую.
...
### `shared/engines/gesture/` (12 файлов, 1145) — движок жестов

- `swipe/usePointerSwipe.ts` (587) — распознавание: фазы
  `idle → press → dragging → cooldown`, окно перехвата 250 мс (`:50-51`:
  «пропускает настоящую прокрутку, ловит удержание»), три скорости по EMA,
...
- `swipe/internals/resolveSwipeDirection.ts` (66) — коммит по флику **или** по
  ...
  **`CONSTRAINT` в коде (`:50-59`)**: во флик-ветке нельзя читать смещение. На
...
### `shared/engines/kinetic/` (26 файлов, 2163) — фасад-сборка
...
- `internal/defaults.ts` (15) — пять значений с объяснением каждого прямо в
  комментарии; окно моментум-глайда и порог «дрожания» — здесь.
- `internal/types.ts` (49) — контракт фасада: `KineticRelease` (что оставил
...
- `internal/motion/compositor/compositedRide.ts` (157 против 296) — урезанный:
...
### `shared/clientState/` (40 файлов) — состояние клиента
...
### `shared/viewportObservation/` (7 файлов, 111)
- `useViewportVisibility` — `IntersectionObserver` плюс `visibilitychange`, и
  ...
  считается видимым. Комментарий `:27-31` объясняет выбор: «всегда видим» стоит
  потребителю лишней работы за экраном, «никогда не видим» тихо выключило бы его
