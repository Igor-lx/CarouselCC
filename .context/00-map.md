...
### `client/areCarouselPropsEqual.ts` (89) — ещё не прочитан
...
### `client/domain/layout.ts` (95) — pure, SSOT геометрии страниц
...
### `client/domain/slides.ts` (84) — pure
...
### `client/domain/track.ts` (53) — pure, но `measureSlotSize` читает DOM
...
### `client/domain/renderWindow.ts` (50) — pure
...
### `client/state/validateState.ts` (68) — pure, dev-only
...
### `client/motion/types.ts` (28) — pure
...
### `client/motion/segmentFactory.ts` (348) — pure, ядро профилирования
...
### `client/motion/useMotionRunner.ts` (273) — fx, самый нагруженный хук слоя
...
### `client/visual-position/useVisualPosition.ts` (139) — fx
...
### `client/visual-position/types.ts` (26), `fallbackPacing.ts` (9), `index.ts` (7)
...
### `client/geometry/useSlotSizeSource.ts` (148) — fx, SSOT измерения слота
...
### `client/geometry/useTrackBinding.ts` (293) — fx, самый «грязный» файл
...
### `client/gesture/useCarouselGesture.ts` (307) — fx
...
### `client/navigation/useCarouselNavigation.ts` (84) — pure-ish
...
### `client/autoplay/useCarouselAutoplay.ts` (61) — fx-композитор
Склейка `useViewportVisibility` + `useViewportBusy` + `useAutoplay`.
`isPaused = !visible || isDragging || isMoving`.

### `client/autoplay/useAutoplay.ts` (92) — fx
Таймер + пауза по ховеру с задержкой.
- `:74-86` `?` при `shouldDeferTick()` вызывается `arm()` заново — то есть
...
### `client/presentation/useCarouselPresentation.ts` (88) — fx (кэш в рефе)
`laneCacheRef` (`:59-83`) — кэш стилей полосы по виртуальному индексу, чтобы
проп `style` у `SlideItem` оставался `===` между двумя перестроениями
`virtualSlides` за поездку. Сброс кэша при смене `layoutOrigin`.
- `?` Кэш чистится **внутри `useMemo`** — вычисление с побочным эффектом на
  внешний реф. Работает, но это скрытая мутация в мемоизации.
...
### `client/context/useModuleContextValue.ts` (138) — pure (мемо-каскад)
**Разделение контекста на два**: `stable` (низкочастотный) и `motion`
(переидентифицируется на каждом переходе). 6 вложенных `useMemo`.
### `client/context/types.ts` (104) — pure. Три вью-контракта + диагностический.
### `client/context/useDiagnosticContextValue.ts` (152) — pure
...
### `client/context/CarouselModuleContext.ts` (29), `CarouselDiagnosticContext.ts` (12)
...
### `client/render-policy/useModuleRenderPolicy.ts` (79) — pure
Гейт слотов: `controls`/`pagination` требуют ещё и `canSlide`;
`diagnostic` — только `IS_DEV`.
### `client/host-report/useCarouselStatusReporter.ts` (40) + `statusSnapshot.ts` (12)
...
### `client/areCarouselPropsEqual.ts` (99) — pure
...
### `client/slides/useCarouselSlideDeck.ts` (66) — pure (4 × `useMemo`)
Цепочка `rawRecords → records (padding) → layout`. Идентичность `layout`
целиком определяется идентичностью пропа `slidesData`.

### `client/slides/useSlideRenderModel.ts` (163) — fx (2 рефа-кэша)
Окно рендера **не сжимается во время движения** (`persistedWindowRef`,
`:44-46`, `:57-64`) — слайд не размонтируется на лету.
...
### `client/slides/useSlideFetchReach.ts` (105) — fx
...
### `client/slides/useOrientationSwapVeil.ts` (79) — fx
...
### `client/slides/SlideItem.tsx` (144) — fx, `memo`
...
### `client/slides/imageResource/createImageResourceStore.ts` (147) — fx, вне React
...
### `client/slides/imageResource/useImageResource.ts` (76) — `useSyncExternalStore`
Корректный мост; `getSnapshot` служит и серверным снапшотом.
### `useImageResourceRetention.ts` (47), `useImageResourceStore.ts` (23),
### `useImageResourceStoreInstance.ts` (31), `types.ts` (26), `index.ts` (9)
...
### `modules/Pagination/basic/usePaginationFade.ts` (555) — fx, второй по сложности
...
### `modules/Pagination/basic/fadeKeyframes.ts` (128) — pure
...
### `modules/Pagination/widget/usePaginationWidgetBinding.ts` (594) — fx, самый большой
...
### `modules/Pagination/widget/math/projection.ts` (86) — pure
...
### `modules/Pagination/widget/math/spatialField.ts` (54) — pure
...
### `modules/Pagination/widget/stepTarget.ts` (59) — pure
...
### `modules/ResponsiveImages/ResponsiveImages.tsx` (110) — fx, headless
...
`Diagnostic.tsx` (111) — 6 групп проверок, каждая под `IS_DEV`, плюс скан
таблиц стилей после mount. Рендерит `null`.
`checks/constantChecks.ts` (826) — аудит **каждой** рукописной константы:
...
`checks/viewportChecks.ts` (241) — сверка осей вьюпорта, парсимости
...
`Carousel.module.scss` (222) в `@layer baseStyles`; модули — в `@layer components`;
...
### `shared/math/numeric.ts` (37) — pure
...
- `runtime/createMotionController.ts` (292) — SSOT позиции. Ключевое:
...
- `profile/profile.ts` (187) — зоны разгон/круиз/торможение, `smoothstep` и его
  интеграл. `:79-82` доли берутся КАК ЕСТЬ, перерасход → отрицательный круиз,
  зона просто пропускается (`pushZone` при `share <= 0`).
- `profile/progressCurve.ts` (169) — плотность стопов выводится из кривой
...
- `compositor/pinnedAnimation.ts` (30) — `fill: "both"` + пин `startTime`,
  два `try/catch` на капризные движки.
- `compositor/compositedRide.ts` (280) — «под ключ» путь для ОДНОГО элемента.
- `runtime/{useMotionController,useMotionPaint,clock,types}.ts` — ленивый реф,
  ref-обёрнутый paint, один клок (`performance.now`).

### `shared/engines/gesture/` (11 файлов, 1058)
- `swipe/usePointerSwipe.ts` (612) — распознавание. Фазы
...
- `swipe/internals/resolveSwipeDirection.ts` (65) — коммит по флику ИЛИ по
...
- `swipe/internals/math.ts` (67), `interactiveTarget.ts` (46) — чистые.
...
Фасад `useKineticValue` (163) поверх ВНУТРЕННИХ форков gesture+motion
...
### `shared/clientState/`
- `shared/useMediaQuery.ts` (78) — ОДИН стор на запрос, count-gated
...
- `media/useMedia/useMedia.ts` (60) — фасад осей. `?` вызывает `useMediaQuery`
...
`resolveSlots` (41) предупреждает в dev о неизвестном и о дублирующем слоте,
берёт последний.
...
`buildSlide` (82) — конвенции одного слайда; `generateSlides` (103) —
идемпотентное слияние с прошлым документом (сохраняет `id` и рукописный `alt`
по стабильному слагу); `runDataGen` (143) — единственный файл, трогающий диск;
`cli.ts` (31) — точка входа.
...
`layoutCssVarsSync` (54), `measurementContractSync` (117), `slideHeightSync` (64),
`styleLayerContract` (97), `orientationMediaSync` (75), `bootSync` (36),
`boundaries` (81), `shippedConstants` (89).
...
### Интеграционный
`carouselContract.test.tsx` (287) — единственный, кто монтирует реальный
...
### Крупнейшие
`reducer.test.ts` (589), `segmentFactory.test.ts` (421),
`usePaginationFade.test.tsx` (422), `useMotionRunner.test.tsx` (336),
`trackBinding.test.tsx` (374), `createMotionController.test.ts` (363).
...
## S. Решения заказчика (действуют постоянно)
