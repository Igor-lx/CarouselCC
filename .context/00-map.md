...
- `runtime/createMotionController.ts` (254) — SSOT позиции.
  ...
  - `?` `destroy()` — **единственный метод контроллера, который зовёт `this`**
    (`this.cancel()`). Все остальные замкнуты на переменные фабрики, поэтому их
    можно отцепить от объекта, а `destroy` — нет: `const { destroy } = ctrl`
    упадёт при вызове. Сегодня его зовут только через объект.
