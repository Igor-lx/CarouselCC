...
- `clientState/shared/` — **единственное, что не дублируется во всём
...
- `shared/useMediaQuery.ts` — один стор на строку медиа-запроса, подписка со
...
- `shared/useMediaQuerySet.ts` — **много условий, одна подписка**. Складывает
