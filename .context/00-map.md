...
- `buildKey` (`:10-17`) — ключ не-клона = `slide:${id}` без индекса, поэтому
  **дубликаты `id` во входных данных дают одинаковые React-ключи**. По ADR-002
  ...
  — а её сегодня не видит никто: ни одна проверка диагностики не смотрит на
  уникальность `id`. Это разрыв в контракте, а не свойство колоды: ADR-002
  обещает наблюдаемость, и здесь она не выполнена. Закрывается проверкой в
  `checks/` — работа не начата, решения по ней нет.
...
- `:47-51` каскад из четырёх источников зазора
  ...
  который верстает вьюпорт своими переменными. Мёртвыми они не являются, но и
  тестом не закреплены.
...
`modules/index.ts` (10) — бочка всех модулей и **мёртвая**: App берёт каждый
модуль глубоким путём. Сверх того она неполна — `ResponsiveImages` в ней нет
вовсе, хотя остальные четыре слота есть.
...
## Q. Тесты — `src/**/tests/**` (112 файлов, 14820)
