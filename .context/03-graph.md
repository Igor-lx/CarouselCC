...
Модули подключаются хостом и **не импортируют ни состояние, ни движение**: всё
через две половины контекста. Отсюда практический вывод: модуль можно удалить
или заменить, не трогая ядро.
...
`client/state/**` — 12 импортёров (+5 тестовых), и зависимость двух разных
сортов:
