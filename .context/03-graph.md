...
| `shared/**` | `src/components/Carousel/**` | — |
...
## Правила изоляции — что слою РАЗРЕШЕНО

Таблица выше перечисляет запреты, и в этом её слабость: она отстаёт от появления
соседней папки, и отстаёт молча. Проверено пробой — у `client/domain/**` в
запретах стояли восемь папок из двадцати с лишним, и импорт в любую из
остальных проходил зелёным. Для слоёв, у которых изоляция — объявленное
свойство, правило поэтому записано наоборот: разрешено перечисленное, всё
прочее запрещено, и новая соседняя папка запрещена сама собой.

| Слой | Может импортировать только |
| --- | --- |
| `client/domain/**` | `shared/**`, `client/public-api/types.ts` |

`domain` — чистое ядро: функции без React, которые зовут откуда угодно и
проверяют без DOM. Из внешнего ему нужны ровно две вещи — общие примитивы полки
и публичные типы, описывающие форму данных хоста.
...
- **`state` не смотрит вниз, в DOM и рендер.** Единственная его зависимость
  вверх — файл `motion/timing.ts`, и тянут его два файла слоя: `reducer.ts`
  (`resolveGoToApproachDistance`) и `transitions.ts` (`resolveGoToPlan`).
  Описана она отдельно.
...
реэкспортит функции полки (`00-map.md`, § E). Позиция всё равно приходит
...
**Общей бочки у модулей нет, и это решение.** Прежняя `modules/index.ts`
удалена: она была мёртвой и неполной, а один корень импорта на все модули ещё и
опасен — он втянул бы диагностику в граф каждого потребителя. App берёт каждый
модуль глубоким путём (запись — `01-facts.md`, C).
...
`motion/index.ts` дополнительно **реэкспортирует часть функций полки**
...
`client/state/**` — 11 импортёров (+10 тестовых), и зависимость двух разных
сортов:
